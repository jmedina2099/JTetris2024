...
public class TetrisController {
	...
	public ResponseEntity<Flux<FigureOperation>> getFigureConversation() {
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.OK)
					.body(fluxOfFigures.delayElements(Duration.ofMillis(125)).timeout(Duration.ofHours(1)));
		} catch (Exception e) {
			this.logger.error("=*=> ERROR: ", e);
	...
	public ResponseEntity<Flux<BoardOperation>> getBoardConversation() {
		...
		try {
			...
			;
			return ResponseEntity.status(HttpStatus.OK)
					.body(fluxOfBoards.delayElements(Duration.ofMillis(125)).timeout(Duration.ofHours(1)));
		} catch (Exception e) {
			this.logger.error("=*=> ERROR: ", e);
			return ResponseEntity.internalServerError().body(Flux.empty());
		}
